...
<%
if(request.getSession().getAttribute("resflag")!=null&&request.getSession().getAttribute("resflag").equals("TRUE"))
{
 request.getSession().removeAttribute("resflag");
 out.println("<script>");
 out.println("window.open('TFsTable.do', '', 'width=800, height=620, toolbar=yes, menubar=yes, resizable=yes, scrollbars=yes');");
 out.println("</script>");
}
%>

<%
   LinkedHashMap<String, ResultTable> rtc=(LinkedHashMap<String, ResultTable>)request.getSession().getAttribute("ResultsTable");
   ResultTable clustab=(ResultTable) request.getSession().getServletContext().getAttribute("clustabs");
   int cutcount=2, topcount=10;
   String varMC=null;
   if(rtc!=null&&rtc.get("tfs")!=null) varMC=rtc.get("tfs").getSingleref();
...
<form id="headerform" action="ResTable.do" method="get" data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-16" data-genuitec-path="/WormExp/WebRoot/table.jsp">
<table id="resheader">
<tr>
...
<%
   ...
   {
	ResultTable rt=rtc.get("org"),rts=rtc.get("tfs");
